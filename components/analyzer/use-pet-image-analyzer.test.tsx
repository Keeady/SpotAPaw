...
const mockUploadMultiplePhotosWithProcessing = jest.fn();
jest.mock("../image-upload-handler", () => ({
  uploadPhotoWithProcessing: jest.fn(),
  uploadMultiplePhotosWithProcessing: () => mockUploadMultiplePhotosWithProcessing(),
}));
  ...
  describe("AnalyzeMultiple", () => {
    ...
    it("should analyze multiple images successfully", async () => {
      const mockResponses = 
        {
          result: '{"pets": [], "image_quality": "good", "number_of_pets": 0}',
          publicUrls: ["https://example.com/photo1.jpg", "https://example.com/photo2.jpg"],
          petDescriptionId: "pet-123",
        }
        
      ;

      mockUploadMultiplePhotosWithProcessing.mockResolvedValue(mockResponses);
      ...
      await act(async () => {
        await result.current.analyzeMultiple([
          { uri: testUri, filename: testFilename, filetype: testFiletype },
          { uri: testUri, filename: testFilename, filetype: testFiletype },
        ]);
      });

      expect(mockUploadMultiplePhotosWithProcessing).toHaveBeenCalledTimes(1);
      expect(mockOnSuccess).toHaveBeenCalledWith(
        JSON.parse(mockResponses.result),
        undefined,
        "pet-123",
     ...
     it("should handle analyzeMultiple with empty images array", async () => {
      const { result } = renderHook(() =>
        usePetAnalyzer({
          onSuccess: mockOnSuccess,
        }),
      ...
      await act(async () => {
        await result.current.analyzeMultiple([]).catch((error) => {
          expect(error).toBeInstanceOf(Error);
          expect(error.message).toBe("No images provided for analysis.");
        });
      });

      expect(mockUploadMultiplePhotosWithProcessing).not.toHaveBeenCalled();
      expect(mockOnSuccess).not.toHaveBeenCalled();
     });

     it("should handle analyzeMultiple with missing parameters", async () => {
      const { result } = renderHook(() =>
        usePetAnalyzer({
          onSuccess: mockOnSuccess,
        }),
      ...
      await act(async () => {
        await result.current.analyzeMultiple([
          { uri: "", filename: testFilename, filetype: testFiletype },
          { uri: testUri, filename: "", filetype: testFiletype },
     ...
     it("should handle analyzeMultiple with upload failure", async () => {
      const mockError = new Error("Upload failed");
      mockUploadMultiplePhotosWithProcessing.mockRejectedValue(mockError);
     ...
     it("should handle analyzeMultiple with invalid JSON in result", async () => {
      const mockResponse = {
        result: "invalid json {",
        publicUrls: ["https://example.com/photo1.jpg", "https://example.com/photo2.jpg"],
        petDescriptionId: "pet-123",
      ...
      mockUploadMultiplePhotosWithProcessing.mockResolvedValue(mockResponse);
     ...
     it("should handle empty response from uploadMultiplePhotosWithProcessing", async () => {
      mockUploadMultiplePhotosWithProcessing.mockResolvedValue(null);
      ...
      await act(async () => {
        await result.current.analyzeMultiple([
          ...
          expect(error).toBeInstanceOf(Error);
          expect(error.message).toBe("Failed with no response");
        });
      });
     ...
     it("should handle missing result and publicUrls but valid petDescriptionId in analyzeMultiple", async () => {
      const mockResponse = {
        result: null,
        publicUrls: null,
        petDescriptionId: "pet-123",
    ...
    it("should handle missing result and petDescriptionId but valid publicUrls in analyzeMultiple", async () => {
      const mockResponse = {
        result: null,
        publicUrls: ["https://example.com/photo1.jpg", "https://example.com/photo2.jpg"],
